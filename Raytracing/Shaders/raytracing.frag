...
uniform float cube_transparency;
uniform float cube_specular_force;
uniform int cube_specular;
...
void initializeDefaultLightMaterials(out SLight light, out SMaterial materials[9]) 
{
	...
	materials[7].RefractionCoef = 0.2;  
	materials[7].MaterialType = DIFFUSE_REFLECTION;

	materials[8].Color = vec3(1.0, 1.0, 1.0); 
	materials[8].LightCoeffs = vec4(lightCoefs); 
    materials[8].ReflectionCoef = cube_specular_force;
	materials[8].RefractionCoef = cube_transparency;
	materials[8].MaterialType = cube_specular;
}
